...
const AddBook = ({ id, setAnnId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id !== undefined && id !== '') {
        await AnnouncementService.updateCon(id, newAnn)
        setAnnId('')
        setMessage({
          error: false,
          message: 'Contact Us Listed',
        })
      } else {
        await AnnouncementService.addCon(newAnn)
        setMessage({
          error: false,
          message: 'New Contact Us Added',
        })
      }
    } catch (error) {
  ...
  const editHandler = async () => {
    setMessage('')
    try {
      const docSnap = await AnnouncementService.getCon(id)
